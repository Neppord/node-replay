...
function parseHeaders(filename, headerLines) {
  var only = arguments[2] === undefined ? null : arguments[2];

  var headers = _Object$create(null);
  var _iteratorNormalCompletion = true;
  var _didIteratorError = false;
  var _iteratorError = undefined;

  try {
    for (var _iterator = _getIterator(headerLines), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      ...
      var _name = _line$match$slice2[0];
      var value = _line$match$slice2[1];

      if (only && !match(_name, only)) continue;

      var key = (_name || '').toLowerCase();
...
function parseRequest(filename, request, requestHeaders) {
  ...
  if (/\sREGEXP\s/.test(methodAndPath)) {
    ...
    var _rawRegexp$match$slice = rawRegexp.match(/^\/(.+)\/(i|m|g)?$/).slice(1);

    var _rawRegexp$match$slice2 = _slicedToArray(_rawRegexp$match$slice, 2);

    var inRegexp = _rawRegexp$match$slice2[0];
    var flags = _rawRegexp$match$slice2[1];
    ...
    var _methodAndPath$split32 = _slicedToArray(_methodAndPath$split3, 2);

    method = _methodAndPath$split32[0];
    path = _methodAndPath$split32[1];
...
function writeHeaders(file, headers) {
  ...
  for (var _name2 in headers) {
    var value = headers[_name2];
    if (only && !match(_name2, only)) continue;
...
function match(name, regexps) {
  ...
  try {
    for (var _iterator3 = _getIterator(regexps), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
  ...
  _createClass(Catalog, [{
    ...
    value: function find(host) {
      ...
      if (stat.isDirectory()) {
        ...
        var mapping = this._read(pathname);
        newMatchers.push(Matcher.fromMapping(host, mapping));
      }
    ...
    value: function save(host, request, response, callback) {
      ...
      try {
        ...
        if (request.body) {
          ...
          try {
            for (var _iterator5 = _getIterator(request.body), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
        ...
        try {
          for (var _iterator6 = _getIterator(response.body), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
    ...
    value: function _read(filename) {
      var _readAndInitialParseFile = readAndInitialParseFile(filename);

      var _readAndInitialParseFile2 = _slicedToArray(_readAndInitialParseFile, 3);

      var request = _readAndInitialParseFile2[0];
      var response = _readAndInitialParseFile2[1];
      var part = _readAndInitialParseFile2[2];
